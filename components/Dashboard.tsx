    ...
    <div className="space-y-12 animate-in fade-in slide-in-from-bottom-8 duration-700">
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          ...
          <p className="text-indigo-400 font-medium mt-2 text-lg">Ready to master a new skill today?</p>
        </div>
        <Button variant="secondary" size="md" onClick={() => onNavigate('library')} className="group">
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {featured.map((companion) => {
          ...
          return (
            <div 
              key={companion.id}
              className={`bg-white border-2 border-indigo-50 rounded-[40px] p-8 relative overflow-hidden flex flex-col h-full shadow-sm hover:shadow-xl hover:shadow-indigo-500/5 hover:-translate-y-1 transition-all group`}
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        {/* Recently Completed */}
        <div className="lg:col-span-2 bg-white rounded-[48px] border border-indigo-50 p-10 shadow-sm">
          <div className="flex items-center justify-between mb-8">
            <h2 className="text-2xl font-black text-indigo-950">Recent Journey</h2>
          ...
          <div className="space-y-6">
            {recentLessons.length > 0 ? (
              recentLessons.map((lesson) => (
                <div key={lesson.id} className="flex items-center group cursor-pointer hover:bg-indigo-50/50 p-4 -m-4 rounded-3xl transition-all">
                  <div className={`w-16 h-16 rounded-2xl ${SUBJECT_METADATA[lesson.subject].color} flex items-center justify-center mr-6 shadow-sm group-hover:scale-105 transition-transform`}>
                    {React.cloneElement(SUBJECT_METADATA[lesson.subject].icon as React.ReactElement, { className: 'w-7 h-7 text-indigo-600' })}
                  </div>
                  <div className="flex-1">
                    <h4 className="font-black text-indigo-950 text-lg">{lesson.companionName}</h4>
                    <p className="text-sm text-indigo-400 font-medium truncate max-w-xs md:max-w-md">{lesson.topic}</p>
                  </div>
                  <div className="flex flex-col items-end gap-1">
                    <span className="text-[10px] font-black uppercase text-indigo-400 bg-indigo-50 px-2 py-0.5 rounded-md">{lesson.subject}</span>
                    <span className="text-xs text-indigo-200 font-bold">{lesson.completedAt}</span>
                  </div>
                </div>
              ))
            ) : (
        ...
        <div className="bg-gradient-to-br from-indigo-950 via-indigo-900 to-purple-900 rounded-[48px] p-10 flex flex-col justify-between text-white relative overflow-hidden group shadow-2xl shadow-indigo-500/20">
          ...
          <div className="relative z-10">
            <div className="bg-white/10 backdrop-blur-md text-white text-[10px] font-black px-4 py-1.5 rounded-full w-fit mb-6 uppercase tracking-widest border border-white/10">
              Personalized
            </div>
            <h2 className="text-3xl md:text-4xl font-black leading-tight mb-4">Create your own <span className="text-indigo-400 italic">Companion.</span></h2>
          ...
          <div className="relative z-10 py-10 flex justify-center scale-125 group-hover:scale-150 transition-transform duration-700">
